  ...
  try {
    ...
    if (booking.status !== "pending") {
      return res.status(400).json({
        error: "This booking is already paid or cancelled",
      });
    }
  ...
  try {
    ...
    if (!booking) {
      return res.status(410).json({
        success: false,
        message: "Booking expired or not found. Please create a new booking.",
      });
    }

    // ⛔ EXPIRED CHECK (IMPORTANT)
