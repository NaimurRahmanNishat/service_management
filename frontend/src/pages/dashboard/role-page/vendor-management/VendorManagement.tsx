// src/pages/dashboard/role-page/vendor-management/VendorManagement.tsx
...
const VendorManagement = () => {
  ...
  const { data, isLoading, error } = useGetAllUsersQuery({role: "vendor"});
  const vendors = data?.data || [];

  /* ================= PAGINATION ================= */
  const totalPages = Math.ceil(vendors.length / ITEMS_PER_PAGE);
  const startIndex = (page - 1) * ITEMS_PER_PAGE;
  const paginatedUsers = vendors.slice( startIndex, startIndex + ITEMS_PER_PAGE );
  ...
  return (
    ...
    >
      <h2 className="text-2xl text-center font-semibold mb-6">
        <AuroraText> All Vendor List</AuroraText>
      </h2>
